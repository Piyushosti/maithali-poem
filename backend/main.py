...
# 3rd version that is faster than 2nd version but still slower than 1st version (uses mps but with some optimizations)
# import os
# import time
# import argparse
# import unicodedata
...
# # 1. APP SETUP
# # ─────────────────────────────────────────────
# app = FastAPI(title="Maithili Nano Poet API - Ultra Fast")
...
# # Load Gemma 2B with LoRA adapter - OPTIMIZED FOR SPEED
# model_gemma = None
# tokenizer_gemma = None

# try:
#     print("\n🔄 Loading Gemma 2B base model with LoRA adapter (8-bit optimized)...")
    
#     from transformers import AutoTokenizer, AutoModelForCausalLM
#     from peft import PeftModel
    
#     # Load base Gemma 2B model with 8-bit quantization for SPEED
#     print("  → Loading base model with 8-bit quantization...")
#     try:
#         # Try 8-bit quantization first (fastest)
#         base_model = AutoModelForCausalLM.from_pretrained(
#             "google/gemma-2-2b-it",
#             device_map="cpu",
#             load_in_8bit=True,  # 8-BIT QUANTIZATION = SUPER FAST
#             torch_dtype=torch.float16,
#             low_cpu_mem_usage=True,
#             trust_remote_code=True,
#             attn_implementation="eager"
#         )
#         print("  → ✅ Loaded with 8-bit quantization (super fast!)")
#     except Exception as e:
#         print(f"  → Fallback to standard loading: {e}")
#         base_model = AutoModelForCausalLM.from_pretrained(
#             "google/gemma-2-2b-it",
#             device_map="cpu",
#             torch_dtype=torch.float32,
...
#     print("✅ Gemma 2B with LoRA adapter loaded successfully (OPTIMIZED)!")
...
# # ─────────────────────────────────────────────
# # 6. GENERATION FUNCTIONS (OPTIMIZED)
# # ─────────────────────────────────────────────
# def generate_with_nano(
#     maithili_word: str,
#     tokenizer,
...
#     device
# ) -> str:
#     """Generate poem using MaithiliNano model - FAST."""
#     try:
#         clean_word = unicodedata.normalize("NFKC", maithili_word)
#         structured_prompt = f"कविता: {clean_word}\n"
...
#         output_ids = model.generate(
#             idx,
#             max_new_tokens=120,  # Reduced from 150 for speed
#             temperature=0.75,
#             top_k=40,
#             top_p=0.92,
#         )
...
#     device
# ) -> str:
#     """Generate poem using fine-tuned Gemma 2B model with LoRA - FAST."""
#     try:
#         prompt = f"""Below is an instruction that describes a task. Write a response that appropriately completes the request.
...
#             outputs = model.generate(
#                 **inputs,
#                 max_new_tokens=150,  # Reduced from 200 for speed
#                 temperature=0.5,
#                 repetition_penalty=1.2,
#                 top_k=30,  # Reduced from 40 for speed
#                 top_p=0.9,
#                 do_sample=True,
...
#             "gemma": model_gemma is not None
#         },
#         "optimization": "8-bit quantization enabled for maximum speed"
#     }


# @app.post("/generate")
# async def generate(req: PromptRequest, request: Request):
#     """Generate poems from one or both models - OPTIMIZED FOR SPEED."""
#     if not req.word.strip():
#         return JSONResponse({"error": "Input word cannot be empty."}, status_code=400)
...
# async def startup_event():
#     print("\n" + "="*70)
#     print("🚀 Maithili Nano Poet API - ULTRA FAST (8-bit Optimized)")
#     print("="*70)
#     print(f"Device: {device} ({device_name})")
#     print(f"MaithiliNano: ✅ Ready (FAST)")
#     print(f"Gemma 2B with LoRA: {'✅ Ready (8-bit OPTIMIZED)' if model_gemma else '⚠️  Not loaded'}")
#     print(f"Expected generation time: 5-10 seconds for both poems")
#     print("="*70 + "\n")
...
# # 5. LOAD MODELS AT STARTUP
# # ─────────────────────────────────────────────
# # Note: add_safe_globals requires torch >= 2.3.0, skipping for compatibility
...
# else:
#     print("⚠️  No HF_TOKEN provided - Gemma may not load")
...
# # ─────────────────────────────────────────────
# # 1. APP SETUP WITH PERFORMANCE OPTIMIZATIONS
# # ─────────────────────────────────────────────
# app = FastAPI(title="Maithili Nano Poet API - Ultra Fast", docs_url=None, redoc_url=None)
...
#             if result and result.strip() and result.strip() != text:
#                 print(f"🌐 Translation via {label}: '{text}' → '{result.strip()}'")
#                 translation_cache[text] = (result.strip(), label)
#                 return result.strip(), label
#         except Exception as e:
#             print(f"⚠️  {label} translation failed: {e}")
#             continue
...
#     print(f"⚠️  All translations failed, using original: '{text}'")
#     translation_cache[text] = (text, "original")
#     return text, "original"
...
# # Load Gemma 2B with LoRA adapter - OPTIMIZED
# model_gemma = None
# tokenizer_gemma = None
...
#     """Generate poem using MaithiliNano model - FAST."""
#     try:
#         clean_word = unicodedata.normalize("NFKC", maithali_word)
#         structured_prompt = f"कविता: {clean_word}\n"
...
# # ─────────────────────────────────────────────
# CULTURAL_NOTES: dict[str, str] = {
#     "घर":    "In Maithili tradition, the home (घर) is a sacred space — a symbol of family, ancestors, and the warmth of Mithila's artistic heritage.",
#     "नदी":   "Rivers hold deep significance in Mithila. The Kamala, Koshi, and Bagmati are not just waterways — they are mothers, celebrated in folk songs.",
#     "फूल":   "Flowers (फूल) are central to Maithili worship and the vibrant Madhubani art tradition that depicts nature in bold, intricate patterns.",
#     "चाँद":  "The moon (चाँद) is a recurring symbol in Maithili poetry, especially in Vidyapati's works, standing as a metaphor for beauty and longing.",
#     "प्रेम": "Love (प्रेम) in Maithili literature is channelled through Vidyapati's devotional verses to Radha and Krishna.",
#     "पानी":  "Water (पानी) is life in Mithila — the rivers, rains, and flooded fields are woven into every aspect of its agriculture and folk memory.",
#     "आकाश": "The sky (आकाश) in Maithili poetry is a canvas for the divine — stars and clouds are read as messages from the gods.",
#     "माँ":   "The mother (माँ) is the most revered figure in Maithili culture, worshipped as the universal mother Durga.",
# }

# def get_cultural_note(maithali_word: str, original_word: str, lang_used: str) -> str:
#     note = CULTURAL_NOTES.get(maithali_word.strip())
#     if note:
#         return note

#     lang_msg = {
#         "maithali": "translated directly into Maithili",
#         "nepali":   "rendered in Nepali Devanagari as a close cultural approximation",
#         "hindi":    "rendered in Hindi Devanagari as a script-compatible fallback",
#         "original": "used as-is since automatic translation was unavailable",
#     }.get(lang_used, "translated")
...
#     return (
#         f"The concept of '{original_word}' was {lang_msg} as '{maithali_word}'. "
#         "Maithili is a language of the Mithila region spanning Nepal and Bihar, India."
#     )
...
# # ─────────────────────────────────────────────
# # 9. BACKGROUND AUDIO GENERATION
# # ─────────────────────────────────────────────
# def generate_audio_background(poem_text: str, audio_path: str):
#     """Generate audio in background without blocking response."""
#     try:
#         for lang_code in ["mai", "hi"]:
#             try:
#                 gTTS(text=poem_text, lang=lang_code).save(audio_path)
#                 print(f"✅ Audio generated: {audio_path}")
#                 return
#             except Exception as e:
#                 print(f"⚠️  TTS with {lang_code} failed: {e}")
#                 continue
#         # Create empty file if all fail
#         open(audio_path, "wb").close()
#     except Exception as e:
...
# @app.post("/generate")
# async def generate(req: PromptRequest, request: Request, background_tasks: BackgroundTasks):
#     """Generate poems from models - ULTRA FAST."""
#     if not req.word.strip():
#         return JSONResponse({"error": "Input word cannot be empty."}, status_code=400)
...
#         # Translate (cached for speed)
#         maithali_word, lang_used = translate_to_maithili(original_word)
#         clean_word = unicodedata.normalize("NFKC", maithali_word)
        
#         responses = {}
        
#         # Generate with MaithiliNano (fast)
#         if "nano" in req.models and model_nano is not None:
#             try:
#                 print(f"🎨 Generating with MaithiliNano...")
#                 poem_nano = generate_with_nano(
#                     maithali_word, 
#                     tokenizer_nano, 
#                     model_nano, 
...
#                 )
#                 responses["nano"] = poem_nano
#                 print(f"✅ MaithiliNano done ({len(poem_nano)} chars)")
#             except Exception as e:
#                 print(f"⚠️  MaithiliNano failed: {e}")
#                 responses["nano_error"] = str(e)
        
#         # Generate with Gemma (if available)
#         if "gemma" in req.models and model_gemma is not None:
#             try:
#                 print(f"🤖 Generating with Gemma 2B...")
#                 poem_gemma = generate_with_gemma(
#                     original_word, 
#                     tokenizer_gemma, 
#                     model_gemma, 
...
#                 )
#                 responses["gemma"] = poem_gemma
#                 print(f"✅ Gemma done ({len(poem_gemma)} chars)")
#             except Exception as e:
#                 print(f"⚠️  Gemma failed: {e}")
#                 responses["gemma_error"] = str(e)
...
#         background_tasks.add_task(generate_audio_background, poem_for_audio, audio_path)

#         base_url = str(request.base_url).rstrip("/")
#         audio_url = f"{base_url}/static/audio/{audio_filename}"

#         cultural_note = get_cultural_note(maithali_word, original_word, lang_used)
        
#         elapsed = time.time() - start_time

#         return {
#             "original_input": original_word,
#             "maithili_input": maithali_word,
#             "language_used": lang_used,
#             "audio_url": audio_url,
#             "cultural_note": cultural_note,
#             "responses": responses,
...
# async def startup_event():
#     print("\n" + "="*70)
#     print("🚀 Maithili Nano Poet API - ULTRA FAST & OPTIMIZED")
#     print("="*70)
#     print(f"Device: {device} ({device_name})")
#     print(f"MaithiliNano: ✅ Ready (FAST)")
#     print(f"Gemma 2B with LoRA: {'✅ Ready (OPTIMIZED)' if model_gemma else '⚠️  Not loaded'}")
#     print(f"Expected response time: 2-5 seconds")
#     print("="*70 + "\n")
...
# cfg = Config()
# model_nano = MaithiliNano(cfg)
# model_nano.load_state_dict(torch.load("./model/finetuned_final.pt", map_location=device))
# model_nano.to(device).eval()
# print(f"✅ MaithiliNano loaded")
...
#     model_gemma = PeftModel.from_pretrained(base_model, "./model/gemma_2b")
#     model_gemma = model_gemma.merge_and_unload()
#     model_gemma.to(device).eval()
...
# # ─────────────────────────────────────────────
# @torch.no_grad()
# def gen_nano(word: str) -> str:
#     try:
#         clean_word = unicodedata.normalize("NFKC", word)
#         prompt = f"कविता: {clean_word}\n"
        
#         bos = tokenizer_nano.token_to_id("<s>")
#         enc = tokenizer_nano.encode(prompt)
#         idx = torch.tensor([[bos] + enc.ids], dtype=torch.long, device=device)
        
#         output_ids = model_nano.generate(idx, max_new_tokens=120, temperature=0.75, top_k=40, top_p=0.92)
#         poem = tokenizer_nano.decode(output_ids[0].tolist(), skip_special_tokens=True)
...
# @torch.no_grad()
# def gen_gemma(word: str) -> str:
#     try:
#         prompt = f"Write a Maithili poem about: {word}\n"
#         inputs = tokenizer_gemma([prompt], return_tensors="pt").to(device)
#         outputs = model_gemma.generate(**inputs, max_new_tokens=150, temperature=0.5, do_sample=True)
#         poem = tokenizer_gemma.batch_decode(outputs, skip_special_tokens=True)[0]
#         return poem.replace(prompt, "").strip()
#     except Exception as e:
#         return f"Error: {str(e)}"
...
# # 8. BACKGROUND AUDIO
# # ─────────────────────────────────────────────
# def gen_audio(text: str, path: str):
#     try:
...
# @app.get("/health")
# def health():
#     return {"status": "ok", "nano": True, "gemma": model_gemma is not None}
...
#         maithali_word, lang = translate_to_maithili(word)
        
#         responses = {}
        
#         # Generate MaithiliNano
#         try:
#             responses["nano"] = gen_nano(maithali_word)
#         except Exception as e:
#             responses["nano_error"] = str(e)
...
#         if model_gemma:
#             try:
#                 responses["gemma"] = gen_gemma(word)
#             except Exception as e:
#                 responses["gemma_error"] = str(e)
        
#         if not responses or all("error" in k for k in responses.keys()):
#             return {"error": "Generation failed"}
        
#         # Audio
#         poem_text = responses.get("nano", responses.get("gemma", ""))
#         audio_file = f"{int(time.time())}.mp3"
#         audio_path = f"static/audio/{audio_file}"
        
#         background_tasks.add_task(gen_audio, poem_text, audio_path)
        
#         base_url = str(request.base_url).rstrip("/")
        
#         return {
#             "word": word,
#             "maithali": maithali_word,
#             "responses": responses,
...
# async def startup():
#     print("\n" + "="*50)
#     print("🚀 Maithili Nano Poet API")
#     print("="*50)
#     print(f"MaithiliNano: ✅")
#     print(f"Gemma 2B: {'✅' if model_gemma else '⚠️'}")
#     print("="*50 + "\n")
...
# app = FastAPI(title="Maithili Nano Poet API - Dual Model Maithili")
...
#     @torch.no_grad()
#     def generate(self, idx, max_new_tokens: int = 120, temperature: float = 0.75, top_k: int = 40, top_p: float = 0.92):
#         for _ in range(max_new_tokens):
#             idx_cond = idx[:, -self.cfg.block_size:]
#             logits   = self(idx_cond)
#             logits   = logits[:, -1, :] / temperature
...
# def translate_to_maithili(text: str) -> tuple[str, str]:
#     """Translate English word to Maithili/Hindi Devanagari script"""
#     attempts = [("maithili", "maithili"), ("ne", "nepali"), ("hi", "hindi")]
#     for target_code, label in attempts:
#         try:
#             result = GoogleTranslator(source="auto", target=target_code).translate(text)
#             if result and result.strip() and result.strip() != text:
...
#     """For Gemma: translate English poem to Hindi/Maithili"""
#     try:
...
#     print("✅ Gemma 2B with LoRA loaded successfully!")
# except Exception as e:
#     print(f"⚠️ Gemma failed: {e}")
#     model_gemma = None
#     tokenizer_gemma = None
...
# @torch.no_grad()
# def gen_nano(word: str) -> str:
#     """Generate Maithili poem using MaithiliNano"""
#     try:
#         clean_word = unicodedata.normalize("NFKC", word)
#         prompt = f"कविता: {clean_word}\n"
...
#         output_ids = model_nano.generate(idx, max_new_tokens=120)
#         poem = tokenizer_nano.decode(output_ids[0].tolist(), skip_special_tokens=True)
        
#         if poem.startswith(prompt):
#             poem = poem[len(prompt):].strip()
        
#         return poem if poem else "कविता निर्माण विफल"
#     except Exception as e:
...
# @torch.no_grad()
# def gen_gemma(word: str) -> str:
#     """Generate poem using Gemma 2B and translate to Hindi/Maithili"""
#     try:
#         # English prompt for Gemma
#         prompt = f"Write a beautiful poem about: {word}\n\nPoem:\n"
#         inputs = tokenizer_gemma([prompt], return_tensors="pt").to(device)
...
#         poem_english = tokenizer_gemma.batch_decode(outputs, skip_special_tokens=True)[0]
#         poem_english = poem_english.replace(prompt, "").strip()
        
#         # Translate English poem to Hindi/Maithili script
#         poem_hindi = translate_english_to_hindi(poem_english)
        
#         return poem_hindi if poem_hindi else poem_english
#     except Exception as e:
#         return f"त्रुटि: {str(e)}"


# def gen_audio(text: str, path: str):
#     """Generate audio from Maithili/Hindi text"""
#     try:
#         for lang in ["hi", "mai"]:
#             try:
...
#             "gemma": model_gemma is not None
#         },
#         "language": "Maithili/Hindi Devanagari"
#     }


# @app.post("/generate")
# async def generate(req: PromptRequest, request: Request, background_tasks: BackgroundTasks):
#     """Generate poems from both models - output in Maithili/Hindi"""
#     try:
#         word = req.word.strip()
#         if not word:
...
#         # Generate with MaithiliNano
#         if "nano" in req.models:
#             try:
#                 responses["nano"] = gen_nano(maithali_word)
#             except Exception as e:
#                 responses["nano_error"] = str(e)
        
#         # Generate with Gemma (translate output to Hindi/Maithili)
#         if "gemma" in req.models and model_gemma:
#             try:
#                 responses["gemma"] = gen_gemma(word)
#             except Exception as e:
#                 responses["gemma_error"] = str(e)
...
#         # Audio from first successful poem
#         poem_text = responses.get("nano") or responses.get("gemma", "")
#         audio_file = f"{int(time.time())}.mp3"
#         audio_path = f"static/audio/{audio_file}"
#         background_tasks.add_task(gen_audio, poem_text, audio_path)
...
#         return {
#             "original_input": word,
#             "maithali_input": maithali_word,
#             "language_used": lang,
#             "responses": responses,
#             "audio_url": f"{base_url}/static/audio/{audio_file}",
#             "output_language": "Maithili/Hindi Devanagari"
#         }
...
# async def startup():
#     print("\n" + "="*60)
#     print("🚀 Maithili Nano Poet - DUAL MODEL (MAITHILI OUTPUT)")
#     print("="*60)
#     print("✅ MaithiliNano: Ready (Maithili output)")
#     if model_gemma:
#         print("✅ Gemma 2B: Ready (Hindi/Maithili output)")
#     else:
#         print("⚠️ Gemma 2B: Not loaded")
#     print("📝 All output in Devanagari script (मैथिली/हिंदी)")
#     print("="*60 + "\n")
...
# def get_cultural_note(word: str, maithali_word: str, lang: str) -> str:
#     """Generate cultural insight about the word"""
#     cultural_notes = {
#         "नदी": "Rivers hold deep significance in Mithila. The Kamala, Koshi, and Bagmati are not just waterways — they are mothers, celebrated in folk songs and the heartbeat of Maithili culture.",
#         "घर": "In Maithili tradition, the home (घर) is a sacred space — a symbol of family, ancestors, and the warmth of Mithila's artistic heritage.",
#         "फूल": "Flowers (फूल) are central to Maithili worship and the vibrant Madhubani art tradition that depicts nature in bold, intricate patterns.",
#         "चाँद": "The moon (चाँद) is a recurring symbol in Maithili poetry, especially in Vidyapati's works, standing as a metaphor for beauty and longing.",
#         "प्रेम": "Love (प्रेम) in Maithili literature is channelled through Vidyapati's devotional verses to Radha and Krishna.",
...
#     # Check if we have a cultural note for this word
#     if maithali_word in cultural_notes:
#         return cultural_notes[maithali_word]
    
#     # Generate a default message
#     return f"The concept of '{word}' was translated to Maithili as '{maithali_word}'. Maithili is a language of the Mithila region spanning Nepal and Bihar, India, rich with the heritage of Vidyapati and the vivid colours of Madhubani art."
...
#         # Get cultural note
#         cultural_note = get_cultural_note(word, maithali_word, lang)
